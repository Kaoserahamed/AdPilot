...
import { activity, initialCampaigns, navItems, type Campaign } from './data';
...
function CampaignsTable({ campaigns }: { campaigns: Campaign[] }) {
  return <section className="panel"><div className="panel-header"><div><h2>Recent campaigns</h2><p>Manage and monitor your latest work</p></div><button className="text-button">View all <Icon name="arrow" size={15} /></button></div><div className="table-wrap"><table><thead><tr><th>Campaign</th><th>Platforms</th><th>Status</th><th>Spend</th><th>Last updated</th><th /></tr></thead><tbody>{campaigns.map((campaign) => <tr key={campaign.name}><td><div className="campaign-name"><i><Icon name="campaign" size={17} /></i><div><strong>{campaign.name}</strong><small>{campaign.product}</small></div></div></td><td><div className="platforms">{campaign.platforms.map((platform) => <span key={platform}>{platform}</span>)}</div></td><td><StatusPill status={campaign.status} /></td><td className="spend">{campaign.spend}</td><td className="updated">{campaign.updated}</td><td><button className="row-menu" aria-label={`Open ${campaign.name}`}>•••</button></td></tr>)}</tbody></table></div></section>;
}
